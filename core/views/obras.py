...
def obra_archivar(request, pk):
    if request.method == 'POST':
        ...
        obra.save()
        accion = 'archivada' if obra.archivada else 'desarchivada'
        messages.success(request, f'Obra "{obra.nombre}" {accion}.')
    return redirect('obras_list')


@login_required
def obra_check_trabajador(request, pk):
    """AJAX: returns vigente contracts for a worker in OTHER obras (not this one)."""
    rut = request.GET.get('rut', '').strip()
    if not rut:
        return JsonResponse({'vigente_otras_obras': []})
    ...
    return JsonResponse({'vigente_otras_obras': data})


@login_required
def obra_asignar_trabajador(request, pk):
    """Create a Contrato (Pendiente de Firma) assigning an existing worker to this obra."""
    if request.method != 'POST':
    ...
    ESTADOS_ACTIVOS = ('Pendiente de Firma', 'Vigente', 'En Licencia', 'Reactivado')
    try:
        trabajador = Trabajador.objects.get(rut=rut, activo=True)
        # Bloquear si ya existe un contrato activo del mismo trabajador en esta obra
        contrato_existente = Contrato.objects.filter(
            trabajador=trabajador, obra=obra,
            estado__in=ESTADOS_ACTIVOS, activo=True
        ).first()
        if contrato_existente:
            messages.warning(
                request,
                f'{trabajador.nombre_completo} ya tiene un contrato activo en esta obra '
                f'(estado: {contrato_existente.estado}). No se puede crear un duplicado.'
            )
            from django.urls import reverse
            return redirect(reverse('obra_detail', args=[pk]) + '?tab=dotacion')
        especialidad = Especialidad.objects.get(pk=int(especialidad_id), activo=True)
        import decimal
        sueldo = decimal.Decimal(sueldo_raw.replace(',', '.')).quantize(
        ...
        messages.success(request, f'{trabajador.nombre_completo} asignado a la obra. Contrato pendiente de firma.')
    except Exception as e:
        messages.error(request, f'Error al asignar trabajador: {e}')
    from django.urls import reverse
    return redirect(reverse('obra_detail', args=[pk]) + '?tab=dotacion')
...
def obra_quitar_trabajador(request, pk, contrato_pk):
    ...
    if request.method == 'POST':
        ...
        if accion == 'eliminar_erroneo':
            ...
            # Redirigir a editar documentación del contrato
            doc = DocumentoGenerado.objects.filter(
                contrato=contrato, tipo='contrato_trabajo', activo=True
            ).first()
            if doc:
                from django.urls import reverse
                return redirect(reverse('doc_generado_edit', args=[doc.pk]))
            else:
                messages.info(request, 'No hay contrato generado para editar. Puede generar uno nuevo desde Documentos.')
...
def obra_detail(request, pk):
    ...
    # Dotación: trabajadores disponibles (exclude already assigned in this obra)
    ruts_en_obra = contratos.values_list('trabajador_id', flat=True)
    trabajadores_disponibles = Trabajador.objects.filter(activo=True).exclude(
        rut__in=ruts_en_obra
    ...
    contratos_con_doc = [(c, _dg_map.get(c.pk)) for c in contratos]

    # Detectar contratos duplicados (mismo RUT, misma obra, ambos activos)
    from collections import Counter
    rut_count = Counter(c.trabajador_id for c in contratos)
